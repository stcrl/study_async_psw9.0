...
def iniciar_desafio(request):
    if request.method == 'GET':
        ...
        if flashcards.count() < int(qtd_perguntas):
            messages.error(
        request, 'Não há flashcards suficientes para a quantidade de perguntas solicitadas adicione mais ou coloque menos flashcards no seu desafio.')
...
def listar_desafio(request, status=None):
    desafios = Desafio.objects.filter(user=request.user)
    if status:
        desafios = desafios.filter(status=status)
